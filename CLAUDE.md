...
3. `compute_haplotypes` → groups phased (or haploid, on chrY non-PAR) variants into haplotype windows using Hail. On chrX non-PAR and chrY non-PAR it applies a haploid-male correction (`_haploid_adjusted_call`, `_carrier_strands`): males contribute a single allele, and chrY keeps `XY` males only. When the caller passes `--min-call-rate`, variants below that fraction of samples called are dropped first (`_filter_low_call_rate`), since missing calls inflate the local AF; on chrY non-PAR only `XY` males count. The workflow passes `hgdp_1kg_min_chry_call_rate` (default 0.8) for chrY only, since the other contigs are imputed. chrY genotypes come from the unphased chrY release VCF (haploid calls need no phasing); that VCF has no records in the PAR regions, so no chrY variant reaching this step is pseudoautosomal.
