...
def test_append_rolls_back_on_streaming_failure(
    ...
    # _stream_tsv_into_sequences (imported by the append tool from divref.duckdb_index) resolves
    # with_compatibility_flag as a module-global in divref.duckdb_index, so that is the module to
    # patch to intercept the real call site.
    import divref.duckdb_index as duckdb_index_mod
    ...
    original = duckdb_index_mod.with_compatibility_flag
    ...
    monkeypatch.setattr(duckdb_index_mod, "with_compatibility_flag", fail_on_rows)
...
_RECORDED_PARAMETERS = hl.Struct(
...
@pytest.mark.parametrize(
    ("contig", "build_parameters", "expected_rows", "expect_warning"),
    [
        pytest.param(
            "chr1",
            _RECORDED_PARAMETERS,
            [("chr1", 0.005, 0.005, 25, 0.8)],
            False,
        ...
        pytest.param("chrX", None, [], False, id="sites_only_contig_records_no_row"),
    ],
)
def test_append_records_haplotype_build_parameters(
    hail_context: None,  # noqa: ARG001
    datadir: Path,
    tmp_path: Path,
    ...
    """Each appended haplotype contig gets one `haplotype_build_parameters` row."""
    ...
    if build_parameters is not None:
        ...
        hl.read_table(str(haplotype_table)).annotate_globals(
            build_parameters=hl.literal(build_parameters)
        ).write(str(annotated))
    ...
    with duckdb.connect(str(_db_path(output_base))) as conn:
        ...
        rows = (
            conn.execute(
                ...
                "min_call_rate FROM haplotype_build_parameters"
            ).fetchall()
            if has_table
            else []
        )
