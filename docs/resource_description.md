...
- chrY non-PAR variants where too few `XY` males have a genotype call are removed (`hgdp_1kg_min_chry_male_call_rate`, default 80%), because chrY genotypes are unimputed and missing calls inflate the local AF.
  chrY empirical AFs come from about 240 to 545 males per population, so they are noisier than on the other contigs.
