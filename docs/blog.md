...
![Autosome (chr1-22) haplotype overlap between the original and new algorithms: 1,947,860 shared, 79,064 original-only, 87,553 new-only](compute_haplotypes/algo_comparison.venn.png)
...
The new algorithm correctly recognizes 78,889 (3.9%) of haplotypes generated by the old algorithm were redundant; their information was entirely contained in longer haplotype that in most cases was already being emitted.
For the 3,969 haplotypes that only had a longer haplotype from the new algorithm, this is due to the counting correctly crediting the longer fragment with carriers from all the parent blocks in which it appears.
